...
- Number.isNaN() // 判断参数是否为 NaN, { (num: number) => boolean }
...
- flat // 默认将二维数组转一维数组, 嵌套层级可通过参数控制, 默认为 1, { (zIndex: number) => Array<any> }
...
- Object.getOwnPropertySymbols // 获取参数对象自身的所有 Symbol 属性名, 存放到数组中返回
...
- Object.setPrototypeOf // 设置对象的原型对象, p1 为目标对象, p2 为原型对象, { (p1, p2) => object }
- Object.keys // 获取参数对象的属性名（不含不可枚举属性）, 存放到数组中返回
...
对象(Object)默认没有部署 Iterator 接口, 原因是对象属性遍历顺序是非线性的  
本质上遍历器是一种线性处理, 对于非线性的数据结构去部署遍历器接口, 就等同于部署一种线性转换  
对象部署遍历器接口不是很有必要, 只有伪数组对象

```js
// 伪数组对象调用数组的`Symbol.iterator`
let iterable = {
  0: 'a',
  1: 'b',
  2: 'c',
...
// 普通对象部署数组的`Symbol.iterator`方法，是无效的
let iterable = {
  a: 'a',
  b: 'b',
  c: 'c',
